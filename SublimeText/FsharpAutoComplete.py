...
if sys.platform == "win32" :
    addUnique(sys.path, os.path.join(os.path.dirname(__file__), "winpexpect"))
    addUnique(sys.path, os.path.dirname(__file__))
    from winpexpect import  winspawn as spawnservice
else :
    addUnique(sys.path, os.path.join(os.path.dirname(__file__), "pexpect"))
    addUnique(sys.path, os.path.dirname(__file__))
    from pexpect import spawn as spawnservice

class AutocompleteService():

    if sys.platform == "win32" :
        prog = '"{0}"'.format(os.path.join(os.path.dirname(__file__), "fsautocomplete\\fsautocomplete.exe"))
        service = spawnservice(prog)#create the process when you create the object
    else:
        prog = os.path.join(os.path.dirname(__file__), "fsautocomplete/fsautocomplete.exe")
        service = spawnservice('/usr/bin/mono "{0}"'.format(prog))#create the process when you create the object
...
class FSharpAutocomplete(sublime_plugin.EventListener):
    ...
    def on_query_completions(self, view, prefix, locations):
        ...
        if (not isFsharp(view.file_name())) : 
            return []
        print("on_query_completions")
